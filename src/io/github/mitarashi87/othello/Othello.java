...
public class Othello {
	...
	private static boolean validateBoadSize(Pos boardSize, int playerCount) {
		if (!validateBoadSizeOnceAxis(boardSize.x(), playerCount)) return false;
		if (!validateBoadSizeOnceAxis(boardSize.y(), playerCount)) return false;
		return true;
	}
	
	private static boolean validateBoadSizeOnceAxis(int onceAxisSize, int playersCount) {
		if (onceAxisSize < playersCount) return false;
		if (playersCount % 2 != (onceAxisSize % 2)) return false;
		return true;
	}
